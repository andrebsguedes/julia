...
function getfield_effects(𝕃::AbstractLattice, arginfo::ArgInfo, @nospecialize(rt))
    ...
    # taint `:noub` if accessing `isbitstype`-type object field that may be initialized
    # with undefined value: note that we don't need to taint `:noub` if accessing
    ...
    # with undefined value to avoid tainting `:noub` too aggressively
    noub = length(argtypes) ≥ 3 && getfield_notundefined(obj, argtypes[3])
